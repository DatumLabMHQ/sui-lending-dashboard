...
export function SuiCards({ kpis: k, asOf }: { kpis: SuiOverview['kpis']; asOf: string }) {
  const stats: Stat[] = [
    ...
    { label: 'Liquidated, 30 days', value: usd(k.liquidated30dUsd), headline: `${k.events30d.toLocaleString('en-US')} liquidation events`, detail: k.unpriced30d ? `Across the four lending protocols; ${k.unpriced30d.toLocaleString('en-US')} more with no usable debt figure left out` : 'Collateral seized across the four lending protocols' },
  ];
  return (
