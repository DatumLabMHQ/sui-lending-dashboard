...
export function SuiCards({ kpis: k, asOf }: { kpis: SuiOverview['kpis']; asOf: string }) {
  const stats: Stat[] = [
    ...
    { label: 'Liquidated, 30 days', value: usd(k.liquidated30dUsd), headline: `${count(k.events30d)} liquidation events`, detail: 'Collateral seized across the four lending protocols' },
  ];
  return (
